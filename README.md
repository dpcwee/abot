...
*[Сводная эксель-таблица доходности]*(https://docs.google.com/spreadsheets/d/1GmRSGlUHzUWBsjTaUjBHimQhNfrCH3BsEIHjQmaETpU/edit?usp=sharing) — Таблица для расчёта примерной будущей доходности бота исходя из текущих значений прибыльности
*[Лесенка усреднений]*(https://docs.google.com/spreadsheets/d/1BGFvGO6_C43Dlv-QDyzzJ8nCRPEb0zk8w8hsmdQlHaE/edit?usp=sharing) — Расчёт будущих усреднений, исходя из параметров buy_down и step_aver
...
*Linux*: В директории с ботом вводите команду на удаление старой версии `rm A-Bot-USDT`,  
затем вводите команду `wget https://github.com/dpcwee/abot/releases/download/{VERSION}/A-Bot-USDT`,  
где вместо ***{VERSION}*** указываете последнюю версию, после чего даёте права файлу `chmod 755 ./A-Bot-USDT`  
и запускаете его командой `./A-Bot-USDT`

### Работа с screen:
1) При первом запуске/перезагрузке сервера вводим `screen -S <name_screen>` (например `screen -S abot`), попадаем сразу в новый screen и запускаем `./A-Bot-BTC` или `./A-Bot-USDT`  
...
5. Создайте публичный или частный канал (Меню -> Создать канал), придумайте его название. Далее, если канал будет публичным, придумайте ему @name (например ***@test_push_abot_channel*** (@name — это название в ссылке t.me/test_push_abot_channel)), добавьте в канал созданного ранее бота ***exambot_abot*** и сделайте его администратором с возможностью изменения профиля канала и публикацией сообщений (первые две галочки возможности администратора). Если же канал будет приватным, то Вам нужно будет получить его chat_id (добавьте Telegram-бота ***exambot_abot*** в частный канал и отправьте там любое сообщение, после чего перейдите в браузере по ссылке `https://api.telegram.org/bot{YOUR_API_KEY}/getUpdates`, где вместо ***{YOUR_API_KEY}*** введите Telegram token, выданный BotFather'ом. Затем среди массива данных найдите ключ "chat" и его значение "id". Оно будет выглядеть примерно как "-1XXXХХХХХХХХХ")  
