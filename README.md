...
* `-lo` — Использовать лимитные ордера вместо рыночных для первых покупок  
...
* Необязательные ключи, которые могут быть дополнением к вашей стратегии (в одном сигнале может находиться несколько ключей или все сразу):  
  ...
  * `'averaging'` ['true'] — В случае, если на активе *уже открыта позиция*, после поступления сигнала с этим ключом бот усреднит её, исходя из торговых параметров. *{"exchange": "{{exchange}}", "symbol": "{{ticker}}", **"side": "BUY"**, "strategy": "my_signal", **"averaging": "true"**}*  
  ...
  * `'update_parameters'` [{'key': 'value', ...}] — Изменить торговые параметры, перечисленные в ключе update_parameters (булевые параметры принимают значения true/false, y/n, yes/no, да/нет). *{"exchange": "BINANCE", **"update_parameters": {"min_bnb": "0.01", "daily_percent": "5", "auto_trade_pairs": "true", "delisting_sale": "no"}**}*
...
* `dev_signals` [y/n] — Открывать позиции по сигналам бота
* `max_dev_signals` [0 < min <= 999] — Максимальное количество позиций, открытых по сигналам бота
* `average_dev_signals` [y/n] — Усреднять открытые позиции по сигналам бота  
...
* `listing_timer` (активируется командой ***--conf listing_timer ЧИСЛО***) — Через какое время в секундах (по умолчанию 3600) снимать с монеты после листинга статус new_listing, если счётчик продаж не достиг max_buy_listing с момента листинга пары
...
* `buy_down` (активируется командой ***--conf buy_down 2.15-3.55-4.8-...-n***) — Ручная сетка процентов падения цены актива от цены последней покупки/усреднения, необходимого для осуществления нового усреднения
