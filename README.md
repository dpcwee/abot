...
При первом подключении к серверу вводим команды `apt update`, а затем `apt-get install screen` и вводим Enter или Y, если этого требует терминал, чтобы обновить системные пакеты и установить менеджер окон (screen)  
1) Вводим `screen -S <name_screen>` (например `screen -S abot`), попадаем сразу в новый screen и запускаем `./A-Bot`  
...
4) В случае перезагрузки сервера (reboot или другой апокалипсис), подключаемся к серверу и повторяем описанный цикл со 1-го пункта  
