...
* `row_sell` [10 >= row_sell >= 1] — Во сколько раз бот может увиличивать множитель **min_order** при открытии позиции в случае, если предыдущая сделка по этой паре закрылась без усреднений или в течение установленного **row_timer** времени? (например 3)  
